...
# Tomographic Imaging, SIAM, Philadelphia, 2001.
    ...
    if len(args) < 2:
        theta = np.matrix(np.arange(1, 181.))
    else:
        theta = args[1]
...
#            yxy = np.delete(yxy,I)
    ...
    A = csr_matrix((vals[:,0].astype(np.float64), (np.squeeze(np.array(rows[:,0]).astype(int)), np.squeeze(np.array(cols[:,0]).astype(int)))), dtype=np.float64, shape=(p*nA, N**2)).toarray()
